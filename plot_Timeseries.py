...
HT_control = np.nanmean(filein[var][:],axis=(1,2))
...
EXP_var = np.empty([nexps,nyears]) * np.nan
...
#                fileinEO =XrayOpen(filenameEO,False)
		...
		HT_exper = np.nanmean(fileinE[var][:],axis=(1,2))
		
		EXP_var[iexp,yearcount] = HT_exper - HT_control
...
for iexp in range(0,nexps):
	if iexp == 0:
		ax.plot(years, np.nanmean(EXP_var[:,:],axis=0), 'r-', label=var, linewidth=2)
		"""
...
"""
ax.grid()
figtitle = Expertype + "CTL-" + Experiments[0] + "Ens_" + var
